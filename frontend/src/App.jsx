...
function App() {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      console.error("Auth Error:", err); // <--- FIXED: Used 'err'
      alert("Auth Failed"); 
    }
  };
  ...
  // --- 2. LOAD DATA ---
  // We define these outside useEffect so we can call them manually if needed
  const fetchProducts = async () => {
    try {
      const res = await axios.get(`http://localhost:3003/products?search=${searchTerm}&category=${categoryFilter}`);
      setProducts(res.data);
      // Auto-select first product for review if list is not empty
      if(res.data.length > 0 && reviewType === 'product') {
         // Only auto-set if targetName is empty to avoid overwriting user selection
         setTargetName(prev => prev || res.data[0].name); 
      }
    } catch (err) { 
      console.error("Fetch Products Error:", err); // <--- FIXED
    }
  };
  ...
  const fetchReviews = async () => {
    try {
      ...
      console.error("Fetch Reviews Error:", err); // <--- FIXED
    }
  };
  ...
  }, [token, searchTerm, categoryFilter, reviewType]); 
  ...
  const seedProducts = async () => {
    try {
      ...
      console.error("Seed Error:", err); // <--- FIXED
    }
  };
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      ...
      console.error("Submit Review Error:", err); // <--- FIXED
              ...
              <div key={p._id} className="product-card">
                ...
                <div className="product-info">
                  ...
                  <div className="product-desc">
                    {p.tags && p.tags.map(t => <span key={t} className="badge" style={{marginRight:'5px'}}>#{t}</span>)}
                  </div>
        ...
        <div className="sidebar">
          ...
          <div className="review-panel" style={{borderTop:'4px solid #4f46e5'}}>
            ...
            <div style={{display:'flex', gap:'10px', marginBottom:'15px'}}>
              <button onClick={()=>setReviewType('product')} style={{background: reviewType==='product'?'#4f46e5':'#ddd', color: reviewType==='product'?'white':'black'}}>Product</button>
              <button onClick={()=>setReviewType('seller')} style={{background: reviewType==='seller'?'#ec4899':'#ddd', color: reviewType==='seller'?'white':'black'}}>Seller</button>
            </div>
